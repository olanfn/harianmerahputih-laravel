...
class ArticleController extends Controller
{
    ...
    private function persist($request, Article $article)
    {
        $data = $request->validated();
        if (blank($data['excerpt'] ?? null)) {
            $bodyText = preg_replace('/<\s*\/?(?:p|br)[^>]*>/i', ' ', (string) $data['body']);
            $data['excerpt'] = Str::limit(trim((string) preg_replace('/\s+/', ' ', strip_tags((string) $bodyText))), 280);
        }
        if (in_array($data['status'], ['published', 'scheduled'], true)) {
            $this->authorize('publish', $article->exists ? $article : new Article);
        }
        if ($data['status'] === 'published' && empty($data['published_at'])) {
            $data['published_at'] = now();
        }
        if (! in_array($request->user()->role, ['super_admin', 'admin'], true)) {
            unset($data['is_editor_pick']);
        } else {
        ...
        return DB::transaction(function () use ($article, $data, $request) {
            $wasNew = ! $article->exists;
            $previousStatus = $article->status;
            $article->fill(collect($data)->except(['tag_ids', 'media'])->all());
            if ($wasNew) {
                $article->author_id = $request->user()->id;
            }
            $article->save();
            ArticleRevision::create(['article_id' => $article->id, 'editor_id' => $request->user()->id, 'title' => $article->title, 'body' => $article->body, 'excerpt' => $article->excerpt, 'status' => $article->status, 'snapshot' => ['category_id' => $article->category_id, 'published_at' => optional($article->published_at)->toISOString()]]);
            AuditLog::record($wasNew ? 'article.created' : ($previousStatus !== $article->status ? 'article.status_changed' : 'article.updated'), $article, ['status' => $article->status, 'is_editor_pick' => $article->is_editor_pick]);
            $article->tags()->sync($data['tag_ids'] ?? []);
            $article->mediaLinks()->delete();
            $featured = false;
            foreach ($data['media'] ?? [] as $position => $item) {
                $role = $item['role'];
                if ($role === 'featured' && $featured) {
                    $role = 'gallery';
                }
                if ($role === 'featured') {
                    $featured = true;
