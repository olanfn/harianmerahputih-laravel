...
<html lang="id">
    @php
        ...
        $structuredData = $isPublishedArticle ? [
            ...
            'publisher' => ['@type' => 'Organization', 'name' => config('news.brand.name')],
        ] : null;
    @endphp
    <head>
        <meta charset="utf-8">
    ...
    <body class="site-shell antialiased">
        ...
        <header x-data="{ menuOpen: false }" class="site-header">
            <div class="info-bar">
                <div class="site-container info-bar__inner">
                    ...
                    <div class="info-bar__right"><span class="social-links" aria-label="Media sosial"><span title="Facebook"><x-icon name="facebook" size="15" /></span><span title="X"><x-icon name="x" size="14" /></span><span title="Instagram"><x-icon name="instagram" size="15" /></span><span title="YouTube"><x-icon name="youtube" size="16" /></span><span title="TikTok"><x-icon name="tiktok" size="15" /></span><span title="RSS"><x-icon name="rss" size="14" /></span></span><form action="{{ route('news.search') }}" method="get" class="header-search"><label class="sr-only" for="header-search">Cari berita</label><x-icon name="search" size="14" /><input id="header-search" name="q" maxlength="100" placeholder="Cari berita, topik, atau kata kunci..." value="{{ request('q') }}"></form></div>
                </div>
            </div>
            <div class="masthead">
        ...
        <footer id="footer" class="site-footer">
            <div class="site-container footer-grid">
                <div class="footer-brand"><a href="{{ route('home') }}" class="brand-lockup brand-lockup--footer"><img src="{{ asset('branding/logo-primary.png') }}" alt="Harian Merah Putih" class="brand-lockup__primary"></a><p>Portal berita Indonesia yang menyajikan kabar, konteks, dan suara dari seluruh negeri.</p></div>
                ...
                <div><h2>Harian Merah Putih</h2><div class="footer-links"><a href="{{ route('institutional.about') }}">Tentang Kami</a><a href="{{ route('redaction.show') }}">Redaksi</a><a href="{{ route('institutional.cyber-media-guidelines') }}">Pedoman Media Siber</a><a href="{{ route('institutional.privacy') }}">Kebijakan Privasi</a><a href="{{ route('institutional.contact') }}">Hubungi Redaksi</a></div></div>
                <div class="footer-follow"><h2>Ikuti Kami</h2><p>Dapatkan kabar pilihan langsung dari ruang redaksi.</p><div class="footer-socials"><span>f</span><span>𝕏</span><span>◎</span><span>▶</span></div></div>
            </div>
