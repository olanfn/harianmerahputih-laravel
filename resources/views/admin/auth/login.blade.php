<x-admin-auth-shell title="Masuk Redaksi">
    ...
    <form method="post" action="{{ route('admin.login.store') }}" class="admin-auth-form">
        ...
        <div class="admin-auth-field"><x-icon name="lock" /><input id="password" name="password" type="password" placeholder="Masukkan password" autocomplete="current-password" required><button type="button" class="password-toggle" data-password-toggle aria-controls="password" aria-pressed="false" aria-label="Tampilkan password" title="Tampilkan password"><x-icon name="eye" size="17" /><x-icon name="eye-off" size="17" /></button></div>
