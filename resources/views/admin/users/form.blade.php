...
<form class="editor-form admin-form-card" method="post" action="{{ $user->exists ? route('admin.users.update', $user) : route('admin.users.store') }}">@csrf @if($user->exists)@method('PUT')@endif
    ...
    <div class="admin-form-grid"><label><span>Password {{ $user->exists ? '(opsional)' : '*' }}</span><input type="password" name="password" minlength="12" @required(!$user->exists) autocomplete="new-password"></label><label><span>Konfirmasi password {{ $user->exists ? '' : '*' }}</span><input type="password" name="password_confirmation" minlength="12" @required(!$user->exists) autocomplete="new-password"></label></div><p class="user-password-help">{{ $user->exists ? 'Kosongkan jika tidak ingin mengubah password.' : 'Gunakan sedikitnya 12 karakter.' }}</p>
